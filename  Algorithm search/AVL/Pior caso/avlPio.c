...
int main(int argc, char **argv) {
    ...
    for (i = 0; i < n; i++) {
        inserir(&raiz, criarNo((rand() % (7001 - 2000)) + 2000), &raiz);
    }

    int numeroAleatorio;
    if (rand() % 2 == 0) {
        numeroAleatorio = rand() % 2001;
    } else {
        numeroAleatorio = (rand() % 3001) + 7000;
    }


    clock_gettime(CLOCK_MONOTONIC, &b);
    struct noArvore** achou = procurar(&raiz, numeroAleatorio);
